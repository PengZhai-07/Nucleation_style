...
H = 0
# L=0.01
LL = [0.025,0.020,0.016,0.012,0.01,0.008,0.006,0.004,0.003]
a = 0.015
sigma = 40e6
...
ax.plot(LL, NS_RA./1000, color="k")

# multiple = [4.0,5.0,6.0,7.0]
# cos_reduction = [0.05,0.06,0.07,0.08]
L = [0.025,0.020,0.016,0.012,0.01,0.008,0.006,0.004]
...
for i in eachindex(L)
    ...
    out_path = "$(@__DIR__)/plots/wholespace/constant_b_0.7143/$(FILE)/"

    NS_width = readdlm(string(out_path, "nucleation info.out"), header=false)
    # NS_width = readdlm(string(out_path, "nucleation info.out"), header=false)
    NS_average = zeros(length(L))
    ...
    # 
    data_path = "$(@__DIR__)/data/wholespace/constant_b_0.7143/$(FILE)/"
    ...
    for j = 1: length(NS_width[:,1])   # plot from the second normal seismic event(third earthquake)
      if i==length(L) 
        ax.plot(L[i], NS_width[j,2], "o", color = color_nucleation[j],  markersize=10, label = j)
      else
        ax.plot(L[i], NS_width[j,2], "o", color = color_nucleation[j],  markersize=10)    
      end    
    end

        # Plot the threotical and average values of nucleation size 
    if i==1
        ax.plot(L[i], NS_average[i], "^", color =color_nucleation[1], markerfacecolor="none", markersize=15, label = "Average value")
        ax.plot(L[i], NS_RA_single/1000, "*", color = color_nucleation[1], markerfacecolor="none",markersize=15, label = "Theoretical-R&A(2005) for a/b>0.5")
    else
        ax.plot(L[i], NS_average[i], "^", color = color_nucleation[1], markerfacecolor="none", markersize=15)
        ax.plot(L[i], NS_RA_single/1000, "*", color = color_nucleation[1], markerfacecolor="none", markersize=15)
...
ax.set_ylim([0,12])
ax.set_xlim([0.026,0.001])
ax.legend(loc="upper right") 
# # # healing analysis: Vfmax and regidity ratio vs. time
#healing_analysis(Vfmax, alphaa, t, yr2sec)
path = "$(@__DIR__)/plots/wholespace/constant_b_0.7143/"        
figname = string(path, "Nucleation size statistics_constant_b_0.7143.png")
